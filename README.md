...
The guard has to catch five different failures, because a check that reads its own pinned input cannot see that input move, a check that names its inputs cannot see the ones it did not name, and a check that looks for a token cannot see the meaning around it change:
