...
11. **Definition lists use explicit markers:** `:: term` then `: definition` (single colon, then a run of one or more spaces - ONE is canonical and the formatter narrows a wider one). Djot's `: term` + indented body does not work.
